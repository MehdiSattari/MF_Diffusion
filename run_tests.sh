...
set -euo pipefail
cd "$(dirname "$0")"
TESTS="test_unet test_encoder test_meanflow test_inference test_regression test_diffusion test_shared_diffusion test_seq2seq test_uncertainty test_ar_convlstm test_ar_ablation"
fail=0
for t in $TESTS; do
  echo "=== tests/$t ==="
