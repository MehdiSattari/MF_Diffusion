...
One-step generation of the next CSI frame:
    ...
    H0 = H1 - u                              # displacement (t-r)*u = 1*u

Since u is the average velocity of the flow H^t = (1-t)Y + t*eps, the endpoint at
t=0 (data) is reached in a single function evaluation: Y_hat = eps - u(eps, Z, 0, 1).

For the AR rollout, the predicted frame is appended to the history and the encoder
is re-run, extending the horizon frame by frame.
"""
...
@torch.no_grad()
def predict_next_frame(generator, z: torch.Tensor,
                       seed_std: float = 1.0, step_noise_std: float = 0.0) -> torch.Tensor:
    """One-step prediction of the next CSI frame from latent Z. Returns [B, 2, Nt, Nc]."""
    B, _, Nt, Nc = z.shape
    device = z.device
    h1 = torch.randn(B, 2, Nt, Nc, device=device) * seed_std     # noise seed H^1
    r = torch.zeros(B, device=device)
    t = torch.ones(B, device=device)
    u = generator(h1, z, r, t)                                   # average velocity
    h0 = h1 - u                                                  # 1-NFE endpoint
    if step_noise_std > 0:
...
def autoregressive_predict(encoder, generator, past: torch.Tensor, num_future: int,
    ...
    The history window grows as predictions are appended (Eq. 23).
    """
    ...
    for _ in range(num_future):
        z = encoder(history)
        nxt = predict_next_frame(generator, z, seed_std, step_noise_std)   # [B, 2, Nt, Nc]
        preds.append(nxt)
